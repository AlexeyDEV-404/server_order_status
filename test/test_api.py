...
async def test_assinged(Create_Session_Factory, new_order, add_master):
    ...
    response = await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/assinged",
        json={"masterID": add_master.json()})
    assert response.status_code == 200
    assert response.json()["status"] == "ASSINGED"


async def test_in_progress(Create_Session_Factory, new_order, add_master):
    await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/assinged",
        json={"masterID": add_master.json()})
    response = await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/in_progress")
    assert response.status_code == 200
...
async def test_completed(Create_Session_Factory, new_order, add_master):
    ...
    await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/in_progress")
    response = await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/completed")
    assert response.status_code == 200
...
async def test_cancel(Create_Session_Factory, new_order):
    response = await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/cancel")
    assert response.status_code == 200
...
async def test_delete_order(Create_Session_Factory, new_order):
    response = await Create_Session_Factory.post(
        f"/user/order/{new_order.json()}/delete")
    assert response.status_code == 200
...
async def test_add_master_skills(Create_Session_Factory):
    response = await Create_Session_Factory.post(
        "/user/insert_skill",
        params={"category": "Электрика", "service": "Ремонт электрощитка"})

    assert response.status_code == 200
    assert response.json() > 0


async def test_insert_skills_and_table_skills(
        Create_Session_Factory, add_master):
    skills = await Create_Session_Factory.post(
        "/user/insert_skill", params={
            "category": "Электрика", "service": "Ремонт электрощитка"
            }
    )
    response = await Create_Session_Factory.post(
        "/user/add_master_skill", params={
