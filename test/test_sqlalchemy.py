...
# (pytest test\test_sqlalchemy.py:
# :test_MastSkillsRep_informarion_about_craftsmen -v -s)
...
async def test_MastListRep_add(add_master):
    ...
    assert isinstance(add_master, int)
    assert add_master > 0
...
async def test_SkillsRepo_insert_works(test_db, insert_works):
    ...
    result = await SkillsRepo(test_db).insert_skill(
...
async def test_MastSkillsRep_add_master_skills(
        test_db, insert_works, add_master):
    x = 1
    result = await MastSkillsRep(test_db).add_master_skills(
        master_id=add_master, skill_id=x)
...
async def test_MastSkillsRep_all_table(test_db, insert_works, add_master):
    await SkillsRepo(test_db).insert_skill(
        category="Сантехника", service="Ремонт трубы")
...
async def test_MastSkillsRep_master_skills(test_db, add_master): 
    await SkillsRepo(test_db).insert_skill(
        category="Сантехника", service="Ремонт трубы")
    await MastSkillsRep(test_db).add_master_skills(
        master_id=add_master, skill_id=1)
...
async def test_MastSkillsRep_search_master(test_db, add_master):
    skil = await SkillsRepo(test_db).insert_skill(
        category="Сантехника", service="Ремонт трубы")
    await MastSkillsRep(test_db).add_master_skills(
        master_id=add_master, skill_id=skil)
    result = await MastSkillsRep(test_db).search_master(
        category="Сантехника", service="Ремонт трубы")
...
async def test_MastSkillsRep_informarion_about_craftsmen(
        test_db, add_master_skills, insert_work
):
...
async def test_OrdRep_update_master_order(test_db, order_one, add_master):
    result = await OrdRep(test_db).update_master_order(
        order_id=order_one, master_id=add_master)
    assert result is not None
    assert result > 0
