...
# pytest test\test_sqlalchemy.py::test_OrdRep_specific_order -v -s
...
def test_MastListRep_add(add_master):
    assert type(add_master) == int
    assert add_master > 0

def test_MastListRep_what_is_the_status(test_db, add_master):
    result = MastListRep(test_db).what_is_the_status(add_master)
    assert result in StatusMasterCheck

def test_MastListRep_master_info(test_db, add_master):
    result = MastListRep(test_db).master_info(add_master)
...
def test_SkillsRepo_insert_works(test_db, insert_works):
    """ Тестируем работу insert_works (вставка скилов в таблицу)."""
    result = SkillsRepo(test_db).insert_skill(category="Сантехника", service = "Ремонт трубы")
    assert result == None or result > 0

def test_SkillsRepo_select_works(test_db, insert_works):
    """ Загружаем данные о навыках через фикстуру и тестируем метод."""
    result = SkillsRepo(test_db).select_works()
    assert len(result) > 0
...
def test_MastSkillsRep_all_table(test_db, insert_works, add_master):
    ...
    test_db.commit()
    add = MastSkillsRep(test_db).add_master_skills(master_id=add_master, skill_id=1)
    result = MastSkillsRep(test_db).all_table()
...
def test_MastSkillsRep_master_skills(test_db, add_master): 
    mast = SkillsRepo(test_db).insert_skill(category="Сантехника", service = "Ремонт трубы")
    add = MastSkillsRep(test_db).add_master_skills(master_id=add_master, skill_id=1)
    result = MastSkillsRep(test_db).master_skills(add_master)
    test_db.commit()
    assert result[0][0].master_id > 0


def test_MastSkillsRep_search_master(test_db, add_master):
    skil = SkillsRepo(test_db).insert_skill(category="Сантехника", service = "Ремонт трубы")
    MastSkillsRep(test_db).add_master_skills(master_id=add_master, skill_id=skil)
    result = MastSkillsRep(test_db).search_master(category="Сантехника", service = "Ремонт трубы")
    
    assert type(result[0].id) == int
    assert result[0].name == 'Андрей' 

def test_MastSkillsRep_informarion_about_craftsmen(test_db, add_master_skills, insert_work):
...
def test_OrdRep_add_order(test_db, order_one):
    
    assert isinstance(order_one, int)
    assert order_one > 0

def test_OrdRep_all_orders(test_db, orders_many):
    result = OrdRep(test_db).all_orders()
...
def test_OrdRep_specific_order(test_db, order_one):
    result = OrdRep(test_db).specific_order(order_one)
    assert isinstance(result.id, int)
    assert result.id > 0
    assert result.description == "Протечка"

def test_OrdRep_master_chek_order_count(test_db, order_one, add_master):
    result = OrdRep(test_db).master_chek_order_count(add_master)
