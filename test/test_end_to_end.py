...
async def test_lifecycle_order(test_db, add_master, insert_work):
    ...
    async with test_db as conn:
        await MastSkillsRep(conn).add_master_skills(
            master_id=add_master, skill_id=insert_work
        )
        order = await OrdRep(conn).add_order(
        ...
        search_by_skil = await MastSkillsRep(conn).search_master(
            category="Сантехника", service="Ремонт трубы"
        )
        await OrdRep(conn).update_master_order(
            master_id=search_by_skil[0].id, order_id=order
        )
        search1 = (await OrdRep(conn).specific_order(order))
        if search1.status != "ASSINGED":
            raise Exception(
                "Ошибка при обновлении статуса заказа с NEW на ASSINGED"
            )
        search2 = await OrdRep(conn).specific_order(order)
        if search2.status != "IN_PROGRESS":
            raise Exception(
                "Ошибка при обновлении статуса заказа с ASSINGED"
                "на IN_PROGRESS"
            )
        search3 = await OrdRep(conn).specific_order(order)
        if search3.status != "COMPLETED":
            raise Exception(
                "Ошибка при обновлении статуса заказа с IN_PROGRESS"
                "в COMPLETED"
            )
        await conn.commit()
