...
def server_order_master_assinged(masterID: int, id_order: int): # Назначаем мастера на заказ, меняем статус 
    with connect_db() as connect:
        master_chek_1 = master_chek_order(connect=connect, id_master=masterID) # Выводим инф., назначен ли мастер
        chek_fetchone_master_order(master_chek_1) # Проверка, назначен ли мастер.
        master_chek_2 = status_select_master(id_master=masterID, connect=connect) # Смотрим статус мастера
        chek_fetchone_master_order(master_chek_2) # Проверяем статус мастера
        assinged_order(connect=connect, id_order=id_order) # Переводи заказ в статус assigned (назначенный)
