...
class Service:
    ...
    async def server_order_create_new(self, category: str, service: str,  description: str):
        """ Создаем новый заказ."""
        add = await self.repo.OrderRepo.add_order(category=category, service=service, description=description, status=StatusOrders.NEW)
    ...
    async def server_order_master_assinged(self, masterID: int, orderID: int):
        ...
        master_count = await self.repo.OrderRepo.master_chek_order_count(master_id=masterID)
        chek_value_master_order_free(master_count)
        
        check_status_per_craftsman = await self.repo.MastListRep.what_is_the_status(id=masterID)
        chek_value_master_order_free(check_status_per_craftsman)

        await self.repo.OrderRepo.assinged_order(order_id=orderID) # Меняем статус заказа 'NEW' -> 'ASSINGED'
        
        try:
            result = await self.repo.OrderRepo.specific_order(order_id=orderID)
        except SQLAlchemyError  as e:
            raise SQLAlchemyError(f'Ошибка запроса на уровне SQLAlchemy. Текст ошибка - {e}')
        else:
            if not result:
        ...
        if result.status != "ASSINGED":
            ...
            raise ValueError((f"Ошибка: полученное значение {result.status} не соответствует ожидаемому ASSINGED."))
       
        mast_ord = await self.repo.OrderRepo.update_master_order(master_id=masterID, order_id=orderID)
        
        if mast_ord is None:
            await self.repo.db.rollback()
            raise ValueError("Ошибка: мастер не назначен за заказ.")
      
        update_status_in_db = await self.repo.MastListRep.update_busy_status_master(id=masterID)
        if update_status_in_db != StatusMasterCheck.BUSY:
            await self.repo.db.rollback()
            raise ValueError(f"Ошибка: полученное значение {update_status_in_db} не соответствует ожидаемому BUSY.")
       
        await self.repo.db.commit()
        return TableOrders.model_validate(result)

    async def server_order_in_progress(self, orderID: int): # Заказ в процессе выполнения
    ...
    async def server_order_complet(self, orderID: int):
        ...
        master = await self.repo.MastListRep.update_free_status_master(id=order.master)
    ...
    async def server_display_master_skills(self):
        ...
        result = await self.repo.MastSkillsRep.informarion_about_craftsmen()    
        if result == []:
            raise ValueError("Error: response cannot be empty")
        return result

    async def server_specific_order(self, orderID: int): # Найти указанный заказ № id_order
        """ Поиск заказа по ID. """
        result = await self.repo.OrderRepo.specific_order(order_id = orderID)
        if result is None:
            raise ValueError(f"Заказ с ID {orderID} не найден")
    ...
    async def server_search_master(self, category:str, service:str):    
        ...
        result = await self.repo.MastSkillsRep.search_master(category=category, service=service)
        if result == []:
    ...
    async def server_cancel_order(self, orderID): # Отмена заказа
    ...
    async def server_add_master_skills(self, master_id: int, skill_id: int) -> TableMasterSkills:
        """ Серверный слой: Вставляем новый навык мастеру в таблицу MasterSkills """
        try:
            result = await self.repo.MastSkillsRep.add_master_skills(master_id=master_id, skill_id=skill_id)
            await self.repo.db.commit()
    ...
    async def server_insert_skill(self, category, service):
        ...
        try:
            result = await self.repo.SkillsRepo.insert_skill(category = category, service = service)   
            await self.repo.db.commit()   
            return result
        except IntegrityError as e:
            await self.repo.db.rollback()
