...
def add_master_skills(master_id, skill_id, db: AsyncSession = Depends(async_get_db)):
...
def insert_skill(category: str, service: str, db: AsyncSession = Depends(async_get_db)) -> int:
...
def order(order_id: int, db: AsyncSession = Depends(async_get_db)):
...
def orders_list(db: AsyncSession = Depends(async_get_db)):
    try: return server_all_orders(db = db)
    except ValueError as e: raise HTTPException(status_code=404, detail=f"{e}")

@router.get("/master_list")
def display(db: AsyncSession = Depends(async_get_db)):
    try: return server_display_master_skills(db = db)
    except ValueError as e:  raise HTTPException(status_code=404, detail=f"{e}")
    
@router.get("/search_master")
def search_master(category: str, services: str, db: AsyncSession = Depends(async_get_db)):
    try: return server_search_master(category=category, service=services, db = db)
    except ValueError as e:  raise HTTPException(status_code=404, detail=f"{e}")

@router.get("/service")
def service(db: AsyncSession = Depends(async_get_db)):
