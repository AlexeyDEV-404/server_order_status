...
from SQLAlchemy_work_db.engine_and_models  import async_get_db
...
async def new_order(user_input: UserInput, db: AsyncSession = Depends(async_get_db)):
...
async def assinged(order_id: int, data: AssingMaster, db: AsyncSession = Depends(async_get_db)):
...
async def in_progress(order_id: int, db: AsyncSession = Depends(async_get_db)):
...
async def completed(order_id: int, db: AsyncSession = Depends(async_get_db)):
    return (await server_order_complet(orderID=order_id, db = db))

@router.post("/order/{order_id}/cancel")
async def cancel(order_id: int, db: AsyncSession = Depends(async_get_db)):
    return (await server_cancel_order(orderID=order_id, db = db))

@router.post("/order/{order_id}/delete")
async def delete_order(order_id: int, db: AsyncSession = Depends(async_get_db)):
    return (await server_delete_order(orderID=order_id, db = db))

@router.post("/add_master")
async def add_master_in_db(name: str, status = StatusMasterCheck.FREE, db: AsyncSession = Depends(async_get_db)) -> int|None:
...
async def add_master_skills(master_id: int, skill_id: int, db: AsyncSession = Depends(async_get_db)):
...
async def insert_skill(category: str, service: str, db: AsyncSession = Depends(async_get_db)) -> int:
...
async def order(order_id: int, db: AsyncSession = Depends(async_get_db)):
...
async def all_table_skills_master(db: AsyncSession = Depends(async_get_db)):
...
async def orders_list(db: AsyncSession = Depends(async_get_db)):
    try: return (await server_all_orders(db = db))
    except ValueError as e: raise HTTPException(status_code=404, detail=f"{e}")

@router.get("/master_list")
async def display(db: AsyncSession = Depends(async_get_db)):
    try: return (await server_display_master_skills(db = db))
    except ValueError as e:  raise HTTPException(status_code=404, detail=f"{e}")
    
@router.get("/search_master")
async def search_master(category: str, services: str, db: AsyncSession = Depends(async_get_db)):
    try: return (await server_search_master(category=category, service=services, db = db))
    except ValueError as e:  raise HTTPException(status_code=404, detail=f"{e}")

@router.get("/service")
async def service(db: AsyncSession = Depends(async_get_db)):
