...
async def new_order(user_input: UserInput,
                    repo: Repository = Depends(get_repository)):
    return (await Service(repo).server_order_create_new(
        category=user_input.category,
        service=user_input.services,
...
async def in_progress(order_id: int,
                      repo: Repository = Depends(get_repository)):
...
async def completed(order_id: int, repo: Repository = Depends(get_repository)):
    return (await Service(repo).server_order_complet(
        orderID=order_id
        )
    )
...
async def cancel(order_id: int,
                 repo: Repository = Depends(get_repository)):
    return (await Service(repo).server_cancel_order(
        orderID=order_id
        )
    )
...
async def add_master_in_db(
    name: str,
    status=StatusMasterCheck.FREE,
    repo: Repository = Depends(get_repository)
) -> int | None:
    try:
        return (await Service(repo).server_add_master_in_db(
            name=name,
            status=status
            )
        )
...
async def add_master_skills(
    master_id: int,
    skill_id: int,
    repo: Repository = Depends(get_repository)
):
    try:
        return (await Service(repo).server_add_master_skills(
            master_id=master_id,
            skill_id=skill_id))
...
async def insert_skill(
    ...
    try:
        return (await Service(repo).server_insert_skill(
            category=category,
            service=service))
    except ValueError as e:
        raise HTTPException(status_code=409, detail=f"{e}")
...
async def order(order_id: int, repo: Repository = Depends(get_repository)):
    try:
        return (await Service(repo).server_specific_order(orderID=order_id))
    except ValueError as e:
        raise HTTPException(status_code=404, detail=f"Данные ошибки: {e}")


@router.get("/table_skill_master")
async def all_table_skills_master(repo: Repository = Depends(get_repository)):
    try:
        return (await Service(repo).server_all_table())
    except ValueError as e:
        raise HTTPException(status_code=404, detail=f"{e}")
...
async def orders_list(repo: Repository = Depends(get_repository)):
    try:
        return (await Service(repo).server_all_orders())
    except ValueError as e:
        raise HTTPException(status_code=404, detail=f"{e}")
...
async def search_master(
    ...
    try:
        return (await Service(repo).server_search_master(
            category=category,
            service=services))
    except ValueError as e:
        raise HTTPException(status_code=404, detail=f"{e}")
...
async def service(repo: Repository = Depends(get_repository)):
    try:
        return (await Service(repo).server_services())
    except ValueError as e:
        raise HTTPException(status_code=404, detail=f"{e}")
